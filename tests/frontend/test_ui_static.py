...
def _parse_static_ui() -> tuple[str, str, str, str, str, UiParser]:
    ...
    observation_format = (root / "python-processor/static/ui/observation-format.js").read_text(encoding="utf-8")
    device_observation_view = (root / "python-processor/static/ui/device-observation-view.js").read_text(encoding="utf-8")
    spectrum_scale = (root / "python-processor/static/ui/spectrum-scale.js").read_text(encoding="utf-8")
    parser = UiParser()
    parser.feed(html)
    return html, css, api_client, observation_format, device_observation_view, spectrum_scale, parser


@pytest.mark.unit
def test_static_ui_contract() -> None:
    html, css, api_client, observation_format, device_observation_view, spectrum_scale, parser = _parse_static_ui()
    ...
    assert '<tbody id="wifiObservationRows"><tr><td colspan="12">Nincs Wi-Fi adat.</td></tr></tbody>' in html
    assert '<th>Feltételezett keretküldő</th>' in html
    assert '<tbody id="wifiSecurityEventRows"><tr><td colspan="10">Nincs Wi-Fi security esemény.</td></tr></tbody>' in html
    ...
    assert "formatPresenceState" not in html and "formatPresenceState" not in observation_format and "formatPresenceState" not in device_observation_view
    ...
    assert {"wifiSessionEmptyState", "wifiSessionContent", "bluetoothSessionEmptyState", "bluetoothSessionContent"} <= parser.ids
    assert {"referenceBar", "referenceBarLabel", "referenceBarSpectrum", "referenceBarWifi", "referenceBarBluetooth"} <= parser.ids
    ...
    for label in ["Pillanatkép rögzítése", "Max Hold rögzítése referenciának", "Referencia mentése", "Betöltés", "Referencia export", "Referencia törlése", "DB referencia-réteg"]:

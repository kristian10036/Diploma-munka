...
def _wifi_baseline_row(**overrides):
    base = {
        ...
        "vendor": "Acme",
        "typical_channel": 6,
        "typical_frequency_hz": 2437000000,
    }
...
def test_wifi_new_device_not_in_baseline_even_with_single_observation():
    ...
    items = [
        _wifi_current_item(
            bssid="11:22:33:44:55:66", stable_identity="11:22:33:44:55:66", observation_count=1
        )
    ]
    result = annotate_devices(cur, items=items, reference_set_id="ref-1", protocol="wifi")
...
def test_wifi_missing_reference_device_listed_separately_not_in_main_items():
    cur = ScriptedCursor(
        [
            [
                _wifi_baseline_row(stable_identity="AA:BB:CC:DD:EE:FF"),
                _wifi_baseline_row(stable_identity="11:22:33:44:55:66"),
            ]
        ]
    )
    items = [_wifi_current_item()]
...
def test_wifi_missing_reference_skips_ignored_baseline_entries():
    cur = ScriptedCursor(
        [[_wifi_baseline_row(stable_identity="11:22:33:44:55:66", expected_state="ignored")]]
    )
    items = []
    result = annotate_devices(cur, items=items, reference_set_id="ref-1", protocol="wifi")
    assert result["reference_missing"] == []
...
def _bt_baseline_row(**overrides):
    base = {
        ...
        "bluetooth_company_id": 76,
        "manufacturer_data_hash": "hash-1",
...
def test_bluetooth_certain_stable_identity_match_is_in_reference():
    ...
    annotate_devices(cur, items=items, reference_set_id="ref-1", protocol="bluetooth")
    assert items[0]["reference_status"] == "in_reference"

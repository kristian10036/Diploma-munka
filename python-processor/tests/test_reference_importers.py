...
class ReferenceImporterTest(unittest.TestCase):
    def test_csv_and_json(self):
        ...
        self.assertEqual(
            json_importer.import_points(
                b'{"points":[{"frequency_hz":1,"power_dbm":-90},{"frequency_hz":2,"power_dbm":-80}]}'
            ).import_format,
            "json",
        )

    def test_peak_is_explicitly_unsupported(self):
        with self.assertRaises(ReferenceImportError) as raised:
            importer_for("sample.peak", "application/octet-stream", b"unknown").import_points(
                b"unknown"
            )

...
from app.schemas import (
    KnownSignalMatchRequest,
    KnownSignalRequest,
    KnownSignalUpdate,
    SpectrumMarkerRequest,
    SpectrumMarkerUpdate,
)
...
def list_spectrum_markers(
    limit: int = 100, measurement_session_id: str | None = None, include_archived: bool = False
):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                f"SELECT * FROM spectrum_markers{where} ORDER BY created_at DESC LIMIT %s",
...
def update_spectrum_marker(marker_id: str, request: SpectrumMarkerUpdate):
    ...
    if "frequency_hz" in changes and (
        changes["frequency_hz"] is None or changes["frequency_hz"] <= 0
    ):
        raise HTTPException(status_code=422, detail="invalid_frequency_hz")
    ...
    query = sql.SQL(
        "UPDATE spectrum_markers SET {}, updated_at = now() WHERE id = %s RETURNING *"
    ...
    if not row:
        raise HTTPException(status_code=404, detail="marker_not_found")
    _write_audit_event(
        "spectrum.marker.updated",
        entity_type="spectrum_marker",
        entity_id=marker_uuid,
...
def archive_spectrum_marker(marker_id: str):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE spectrum_markers SET archived_at = now(), updated_at = now() "
    ...
    _write_audit_event(
        "spectrum.marker.archived", entity_type="spectrum_marker", entity_id=marker_uuid
    )
    return row
...
def _validate_known_signal(request: KnownSignalRequest | KnownSignalUpdate) -> None:
    ...
    if "label" in values and (
        values["label"] is None or not values["label"].strip() or len(values["label"]) > 200
    ):
...
def list_known_signals(limit: int = 100, status: str | None = None, include_archived: bool = False):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute(
                f"SELECT * FROM known_signals{where} ORDER BY updated_at DESC LIMIT %s",
                (*values, bounded_limit),
            )
            rows = cur.fetchall()
    return {"items": rows, "count": len(rows)}
...
def create_known_signal(request: KnownSignalRequest):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                INSERT INTO known_signals
                  (location_id, measurement_session_id, center_frequency_hz,
                   frequency_tolerance_hz, bandwidth_hz, expected_power_min_dbm,
                   expected_power_max_dbm, modulation, protocol, source_type, label, notes,
                   status, suppress_alerts, valid_from, valid_until, metadata)
            ...
            """,
                (
                    ...
                    request.frequency_tolerance_hz,
                    request.bandwidth_hz,
                    request.expected_power_min_dbm,
                    request.expected_power_max_dbm,
                    request.modulation,
                    request.protocol,
                    request.source_type,
                    request.label.strip(),
        ...
        conn.commit()
    _write_audit_event(
        "spectrum.known_signal.created",
        entity_type="known_signal",
        entity_id=str(row["id"]),
        details={"center_frequency_hz": request.center_frequency_hz},
    )
    return row
...
def get_known_signal(known_signal_id: str):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT * FROM known_signals WHERE id = %s", (signal_uuid,))
            row = cur.fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="known_signal_not_found")
    return row
...
def update_known_signal(known_signal_id: str, request: KnownSignalUpdate):
    ...
    assignments = [sql.SQL("{} = %s").format(sql.Identifier(field)) for field in changes]
    query = sql.SQL(
        "UPDATE known_signals SET {}, updated_at = now() WHERE id = %s RETURNING *"
    ).format(sql.SQL(", ").join(assignments))
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute(query, (*changes.values(), signal_uuid))
            row = cur.fetchone()
        conn.commit()
    if not row:
        raise HTTPException(status_code=404, detail="known_signal_not_found")
    _write_audit_event(
        "spectrum.known_signal.updated",
        entity_type="known_signal",
...
def archive_known_signal(known_signal_id: str):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE known_signals SET archived_at=now(), status='disabled', updated_at=now() "
                "WHERE id=%s AND archived_at IS NULL RETURNING *",
                (signal_uuid,),
            )
            row = cur.fetchone()
        conn.commit()
    if not row:
        raise HTTPException(status_code=404, detail="known_signal_not_found_or_archived")
    _write_audit_event(
        "spectrum.known_signal.archived", entity_type="known_signal", entity_id=signal_uuid
    )
    return row
...
def match_known_signal(request: KnownSignalMatchRequest):
    ...
    matches = [
        evaluate_known_signal(profile, measurement) | {"label": profile["label"]}
